...
ELEMENT_META = {
    "木": {"pair": "zx ↔ nx", "yang": "阳木", "yin": "阴木", "color": "4",
          "variable": "最终方向与作用权来源"},
    "火": {"pair": "zn ↔ x", "yang": "阳火", "yin": "阴火", "color": "1",
          "variable": "对象独立成立资格与归属方式"},
    "土": {"pair": "n ↔ x并z", "yang": "阳土", "yin": "阴土", "color": "6",
          "variable": "对象跨越内外边界的方向"},
    "金": {"pair": "xn ↔ z", "yang": "阳金", "yin": "阴金", "color": "5",
          "variable": "运行权与裁定重心配置"},
    "水": {"pair": "xz ↔ nz", "yang": "阳水", "yin": "阴水", "color": "2",
          "variable": "可逆性与对象特异回返空间"},
}
...
def parse_mapping() -> dict[str, dict[str, str]]:
    ...
    if actual != expected:
        ...
        raise ValueError(f"五行总表不是完整98项；缺失={missing}，异常={extra}")
    return mapped
...
def load_evidence() -> tuple[dict[str, dict[str, Any]], dict[str, dict[str, str]]]:
    ...
    for number, info in mapped.items():
        ...
        if not item["image"] or not item["source"]:
            raise ValueError(
                f'{number} {item["name"]} 缺少图片或来源页。'
                "S级图片Skill采用fail-closed，禁止生成文字空壳。"
            )
...
def add_card_grid(
    ...
    nodes.append({
        "id": group_id,
        "type": "group",
        "label": f"{group_label}｜{len(items)}项｜配图证据版",
        "x": group_x,
        "y": y_start,
        "width": group_w,
        "height": group_h,
    ...
    for index, item in enumerate(items):
        ...
        nodes.append({
            "id": f'card-{item["number"]}',
            "type": "text",
            "text": card_text(item),
            "x": group_x + 80 + col * (card_w + gap_x),
            "y": y_start + 120 + row * (card_h + gap_y),
            "width": card_w,
            "height": card_h,
            "color": color,
        })
...
def build_element_canvas(
    ...
    nodes: list[dict[str, Any]] = [{
        ...
        "type": "text",
        "text": (
            f'# {element}库｜{meta["pair"]}｜配图证据版\n\n'
            f'共{len(items)}项：{meta["yang"]}{len(yang_items)}项／'
            f'{meta["yin"]}{len(yin_items)}项。\n\n'
            f'**同轴变量：** {meta["variable"]}\n\n'
            '**证据口径：** 每项已经显示一张真实来源参考图；'
            '但“有图”不等于“S级通过”。未完成90–100分S图或两张互补A图核验的条目，'
            '统一保留 `evidence-pending`，禁止冒充canonical。\n\n'
    ...
    next_y = add_card_grid(
        nodes, yang_items, "group-yang", meta["yang"], 0, meta["color"]
    )
    add_card_grid(
        nodes, yin_items, "group-yin", meta["yin"], next_y, meta["color"]
    )
    edges.extend([
        {"id": "edge-title-yang", "fromNode": "title", "toNode": "group-yang"},
        {"id": "edge-yang-yin", "fromNode": "group-yang", "toNode": "group-yin",
         "label": f'{element}轴｜阴阳对立统一'},
    ])
...
def build_image_index(evidence: dict[str, dict[str, Any]]) -> None:
    ...
    positions = {
        "木": (-1900, 0), "火": (-950, 0), "土": (0, 0),
        "金": (950, 0), "水": (1900, 0),
    }
    paths = {
        "木": "五行库/木/木_zx-nx.canvas",
        "火": "五行库/火/火_zn-x.canvas",
        "土": "五行库/土/土_n-x并z.canvas",
        "金": "五行库/金/金_xn-z.canvas",
        "水": "五行库/水/水_xz-nz.canvas",
    }
    for element in "木火土金水":
        first = sorted(
            (item for item in evidence.values() if item["element"] == element),
        ...
        nodes.append({
            ...
            "type": "text",
            "text": (
                f'## {element}库｜{ELEMENT_META[element]["pair"]}｜{counts[element]}项\n\n'
                f'![]({first["image"]})\n\n'
                f'[[{paths[element]}]]\n\n'
                f'样例：{first["number"]} {first["name"]}\n'
                '状态：配图完成，S级逐图审计待完成。'
            ),
            "x": x,
            "y": y,
    ...
    (REORG / "00_五行配图总索引.canvas").write_text(
        json.dumps(
            {"nodes": nodes, "edges": edges,
             "metadata": {"version": "1.0-1.0", "frontmatter": {}}},
            ensure_ascii=False,
...
def build_audit(evidence: dict[str, dict[str, Any]]) -> None:
    ...
    for number in sorted(evidence):
        ...
        rows.append(
            f'| {number} | {item["name"]} | {item["element"]} | '
            f'{item["polarity"]} `{item["ten_yuan"]}` | {item["grade"]} | {score} | '
            f'`{item["audit_status"]}` | [来源]({item["source"]}) |'
        )
...
“图片已经显示”和“图片证据已经达到S级”是两件事。前者解决本轮缺图，后者仍须执行来源权威、结构可见度、名称一致、比较价值、去重和链接稳定六项评分。终于把“有图”与“图靠谱”分开了，仓库暂时避免了视觉版自欺欺人。
...
def rebuild_main_index() -> None:
    ...
    for element in "木火土金水":
        ...
        nodes.append({
            ...
            "type": "text",
            "text": (
                f'## {element}｜{ELEMENT_META[element]["pair"]}｜{counts[element]}项\n\n'
                f'[[{paths[element]}]]\n\n'
                f'变量：{ELEMENT_META[element]["variable"]}\n'
                '内容：编号、图片、原始来源、图片等级、证据状态。'
            ),
            "x": positions[element],
            "y": 0,
            "width": 760,
    ...
    (REORG / "00_五行总索引.canvas").write_text(
        json.dumps(
            {"nodes": nodes, "edges": edges,
             "metadata": {"version": "1.0-1.0", "frontmatter": {}}},
            ensure_ascii=False,
...
def main() -> None:
    ...
    paths = {
        "木": REORG / "五行库/木/木_zx-nx.canvas",
        "火": REORG / "五行库/火/火_zn-x.canvas",
        "土": REORG / "五行库/土/土_n-x并z.canvas",
        "金": REORG / "五行库/金/金_xn-z.canvas",
        "水": REORG / "五行库/水/水_xz-nz.canvas",
    }

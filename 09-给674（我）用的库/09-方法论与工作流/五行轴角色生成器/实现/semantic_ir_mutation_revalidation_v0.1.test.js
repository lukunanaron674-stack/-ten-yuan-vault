...
const { revalidateMutationCandidate, MUTATION_REVALIDATION_VERSION, GATE_SOURCE_STATUS, EXECUTION_MODE } = require('./semantic_ir_mutation_revalidation_v0.1.js');
...
test('raw candidate_gates compatibility path is explicitly unverified', () => { const r=revalidateMutationCandidate(candidate(),'候选',opts({execution_mode:EXECUTION_MODE.COMPATIBILITY})); assert.strictEqual(r.status,'PASS'); assert.strictEqual(r.gate_source.status,GATE_SOURCE_STATUS.UNVERIFIED_GATE_SOURCE); });
...
test('production mode rejects raw candidate_gates', () => assert.throws(()=>revalidateMutationCandidate(candidate(),'候选',opts({execution_mode:EXECUTION_MODE.PRODUCTION})), e=>e.code==='ERROR_IR_REVALIDATE_VERIFIED_GATE_REGISTRY_REQUIRED'));
test('production mode accepts registry-verified gates', () => { const r=revalidateMutationCandidate(candidate('organization'),'组织审批结构',registryOpts(registry(),{execution_mode:EXECUTION_MODE.PRODUCTION})); assert.strictEqual(r.status,'PASS'); assert.strictEqual(r.symbol,'xn'); assert.strictEqual(r.gate_source.status,GATE_SOURCE_STATUS.REGISTRY_VERIFIED); });
test('unknown execution mode is hard rejected', () => assert.throws(()=>revalidateMutationCandidate(candidate(),'候选',opts({execution_mode:'silent-auto'})), e=>e.code==='ERROR_IR_REVALIDATE_EXECUTION_MODE_UNKNOWN'));

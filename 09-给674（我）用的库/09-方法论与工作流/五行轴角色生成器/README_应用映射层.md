...
```text
最终方向、权限或解决通道先由外部真实建立
主体可以自愿接入并积极执行，但不占最终重设/否决/改题位置
借用他者权限不等于权限所有权过户
沿关系网络借力要求关键节点保持独立，不归主体掌握
外部给目标不自动等于 nx；主动搭流程优先检查 xn
强迫、职位、贫弱、服从与“求人”标签均不能单独成立 nx
```

`nx` 世界观生成的是“可接入的既有方向与权限接口”，不是把世界里的所有下属、员工或弱者判成 `nx` 人物。若主体自行重设最终方向并迫使世界响应，优先检查同轴 `zx`；若对象/权限归主体所有，检查 `x`；若接到目标后主动组织计划、节点和资源，检查 `xn`。
...
- nx 世界观：5 条，全部 `candidate`。

共 30 条运行时映射，其中 z 默认不进入生产池。机器 JSON 只是 Markdown 应用映射投影，不拥有更高理论权限。

世界观横向覆盖现已达到 6 个十元位置；R2 60+ 运行时压力测试的覆盖前提已满足，但压力测试属于执行层独立工作，不在映射研发中冒充已完成。

## 8｜下一轮

理论映射轮转按覆盖最少进入：`xn × 世界观`。

执行层并行优先级：

1. 继续补真实 `genre_translation`，避免只换 genre 标签；
2. 以当前 6 个世界观十元位置启动/继续 R2 60+ 运行时压力测试；

...
- 护栏：孙悟空弃弼马温 99/98；贾政受贾母现实否决 99/98。
...
- 正向：鲁智深 99/97；鸳鸯 99/96；刘备携民渡江 99/96。
- 护栏：杨志押送生辰纲 99/98，高责任/高代价可由外部委任、功名/恩遇、问责和惩罚压力解释。
- 待审议：`待审议问题_zn-x纯zn独立成立与责任对象不等于现实x边界_20260827.md`。

### A4｜zn+x 当前共现
`3 positive controls / 3 works + 6 negative guards / 4 guard works / pending-review`
- 正向：关羽、晁盖、唐僧。
- 六类护栏：`x=true/zn不过门`；名位假 `x`；责任对象≠掌握对象；第三因素足以解释；跨阶段端点拼接；第三方 veto 不能记作主体 `x`。
...
#### verified positive
- 诸葛亮卤城换班 99/95：第二次对抗审计已冻结士气/长期信誉收益，仍暂时守住。士气收益发生在首次放行之后，且军士第一次愿留时诸葛亮仍要求其还家；不增加 positive 计数，不上调分数。
...
3. 严颜：real `x` loss 后主体身体/生命/跪降选择仍承载 `zn`。
4. 晁盖“不伤无关者”：execution/amplification interface。

#### strict precondition guards｜6 controls / 3 works
1. 刘备：跨阶段 pure-x/pure-zn 不能拼 strict。
2. 鸳鸯：第三方 veto 不能记作主体 `x`。
3. 宋江陈桥驿 99/98：军纪 `x` 真，候选 `zn` 可被法度/招安/追责/连累解释。
4. 王熙凤协理宁国府 99/98：局部处分 `x` 真，候选 `zn` 可被受托职责/治理绩效/制度威慑解释。
5. 柴进庄园庇护 99/97：庄园、钱粮、执行关系、书信/人情是不同现实载体，不能事后捆绑为无限宽 composite `x` 制造 `x→zn` 缺口。
6. 曹操割发代首 99/98：军纪处分 `x` 真，但公开自罚可由服众、军令可信度、治理威慑与角色绩效解释，候选 `zn` 未独立过门。

作品去重：刘备+曹操=`三国演义`；鸳鸯+王熙凤=`红楼梦`；宋江+柴进=`水浒传`，故6 controls仍为3 works。
...
strict_legacy_v1_negative_guard_works: 0
strict_precondition_guards: 6
strict_precondition_guard_works: 3
...
1. 放弃/失去 `x` ≠ 自动 `zn`。
2. 已成立 `x` 被限制 ≠ 自动 `zn`；先查限制来源。
3. 外部封号/认可/身份变化 ≠ `zn` 被改写。
4. 职位、印信、代理授权 ≠ 稳定 `x`。
5. 保护/责任/不抛下/公共责任 ≠ 被负责对象进入 `x`。
6. `zn` 限制 `x` 使用方式，不等于 `x` 不成立。
7. `zn+x` 共现前，两端必须分别独立过 current canonical。
8. 跨阶段、跨对象层的高纯端点不能拼成当前共现。
9. 最终结果与 `zn` 同向 ≠ 主体拥有使结果生效的 `x`；第三方 veto 必须归第三方。
10. co-occurrence ≠ strict complement。
11. strict 同 criterion_version 才累计跨作品。
12. current reality anchor ≠ absolute metaphysical uniqueness。
13. strict `x→zn` 禁止循环定义：先用不引用被测 `x` 的语言独立命名 `zn`。
14. 共享治理 `x` 不能私有化给单一领导者。
15. 高纯 P1 `zn` 不能跨原则倒灌。
16. `self-restraint-looking use of x ≠ independent zn`。
17. local `x` ≠ global `x`；future revocability ≠ prior current x never existed。
18. 当前 `x` 关闭 ≠ `zn` 未来调用资格删除；`x` 持续 on ≠ `zn` 永久持续。
19. historical `zn` ≠ permanent identity。
20. recognition time ≠ establishment time。
21. 高责任/高代价 ≠ `zn`。
22. 真实第三因素足以解释同一结果时，不重复拿结果证明 `x`。
23. pressure-display 解压后必须 same principle + same boundary + same conflict-order criterion。
24. evidence-locked 可被对抗审计撤回。
25. control 数与 independent work 数分开。
26. execution/amplification/screened resource/old governance object 不自动等于 strict current anchor。
27. `real x loss ≠ automatic x→zn gap`。
28. 多个不同 `x` 共同承载同一原则时默认分别测，禁止按目标结果事后拼装 composite `x`。
29. 公开自罚/自限与治理收益同向时，必须先冻结服众、信誉、绩效和威慑等第三因素。
...
1. **P0：继续攻击诸葛亮 99/95，但换攻击机制。** 士气/感恩收益已完成二次攻击且不足替代 `zn`；下一步优先统军制度强约束、原则宽化后的 competing anchors。
2. **P1：探春二审**，只解 `criterion_identity_across_contexts`。
3. **P2：晁盖 former-positive 二审**，只解共同公库 subject-specific `x` 归因。
4. **P3：第二独立作品、不同 `x` 类型的 strict v2 positive。** 被测 `x` 必须自然可识别，禁止 posthoc bundling。
5. 成熟槽只补新型反例；不重复“职责/绩效”“第三方 veto”“跨阶段拼接”“composite-x bundling”“治理信誉型自罚”等已锁机制。
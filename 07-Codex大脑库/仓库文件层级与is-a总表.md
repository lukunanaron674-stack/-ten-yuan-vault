...
| `01-十元系统/关系专项/README_十元关系审计索引.md` | 字典或总表 | 未锁定审计、历史证据与纠正补丁路由 | 专项正本索引 | 未锁定审计、二次/三次审计、已覆盖旧审计、孤立旧审计 | current v0.8 | audit-routing | 专项正本索引 | 审计文件读取身份 | `xn克zn` 未被核心表 v2.22 登记，已按孤立历史候选收口 | 总入口→核心表→专项索引→审计索引 | 本轮提交后回读核验 |
...
| `01-十元系统/关系专项/xn克zn_结构压缩意义实践还是改写内核机制审计_20260720.md` | 旧稿 | 非现行几何位候选证据 | 无；核心表 v2.22 未登记此边 | 无 | legacy-evidence-only | L6 | 核心表、审计索引、分散补记消化摘要 | 结构压缩意义实践通道的历史候选 | 不得恢复 `xn克zn` 几何位或写成克制 zn 内核 | 专项索引→审计索引→旧证据 | 本轮提交后回读核验 |
| `07-Codex大脑库/十元理论每小时运行记录.md` | 自动运行记录 | 十元理论唯一活动账本 | 强制必读中枢 | Git 历史与 previous_active_blob | active v11.04 | highest-log | 无 | 当前理论运行状态 | 97 份旧分散补记已消化删除，不得重建并列账本 | 强制清单→理论账本 | 本轮起始 main 回读 |
| `07-Codex大脑库/归档/十元理论每小时分散补记消化摘要_20260717-20260722.md` | 归档 | 97 份分散补记的历史消化摘要 | 当前理论运行账本 | Git 历史原文 | archived-consolidated | L6 | 当前账本、核心表、专项正本、审计索引 | 旧补记形成史与覆盖计数 | 不恢复旧 current/complete 状态，不生成关系 | 当前理论账本→历史摘要→Git历史 | 本轮提交后回读核验 |
...
| `07-Codex大脑库/仓库文件层级与is-a总表.md` | 字典或总表 | 仓库治理总表 | 总入口 | 治理运行记录 | current v3.2 | governance | 无 | 历史治理快照 | 无 | 总入口→治理总表 | 本轮提交后回读核验 |
| `07-Codex大脑库/仓库层级清理运行记录.md` | 自动运行记录 | 仓库治理唯一活动账本 | 治理总表 | 历史治理归档 | active v3.3 | log | 无 | 历史治理快照 | 无 | 治理总表→运行账本 | 本轮提交后回读核验 |
| `00-中枢索引/AI文件权力与任务总览.md` | 治理地图 | 文件权力与自动任务读写路由 | `AGENTS.md` | `总入口.md`、配套 Canvas | canonical-current v1.2 | L3 governance | 无 | 旧 Vault 总览与旧入口的现行权力 | 不覆盖 L2 理论正本 | AGENTS→权力地图→总入口 | 本轮提交后回读核验 |
| `00-中枢索引/总入口.md` | 总入口 | 全库唯一现行人类导航 | AI 文件权力与任务总览 | 各现行专项入口 | current v2.0 | L3 navigation | 无 | `总入口_更新版.md` 及两个幽灵入口 | 不定义理论 | 权力地图→总入口→专项正本 | 本轮提交后回读核验 |
...
8. `00-中枢索引/` 已按 3 个现行中枢、4 个兼容路由、1 个 L5 材料、16 个 L6 历史材料收口；目录位置不再自动赋权。
9. 97 份理论运行补记已消化为 1 份 L6 摘要并从活动目录删除；现行账本继续保持唯一。
10. `xn克zn` 孤立旧审计已降为 L6；核心表未登记该边，不能由旧补记反向恢复。

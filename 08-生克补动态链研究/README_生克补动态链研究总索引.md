---
type: dynamic-chain-master-index
status: canonical-current
version: v3.93
updated: 2026-09-17
framework_version: v3.94
execution_protocol_version: v1.4
method_amendment_version: v2.1.2-routing-priority-hotfix
template_version: v1.3
---

# 生克补动态链研究总索引

## 2026-09-17 研发补充入口

> 以下为 2026-09-16→17 新增的研发层，不覆盖本索引既有影片审计正本与十元 canonical。

- `VERSION_动态链系列研发.yaml`：当前动态链系列所有子系统版本与下一里程碑。
- `DAILY_20260916-17_动态链系列研究知识包.md`：今日所有动态链相关新成果总整理。
- `动态链质量评估器_v1.0_20260917.md`：节点20 / 边40 / 整链40 的质量评价器；R1–R4冻结，R5待盲测。
- `NEXT_动态链系列研究队列_20260917.yaml`：R5经典作品盲测、ZX/NX端点验证、生克补T1、Dynamic Visual盲测、黎黎隆R6的机器可读续跑队列。
- `01-十元系统/06-视觉化研发/03-十元生克补/`：Relation Visualization v0.1。
- `02_给我用的知识凝结库/01_动态链条/视觉化/`：Dynamic Visual Operations v1。
- `[[02_给我用的知识凝结库/01_动态链条/00_动态链条总控_零件关系与完成度.canvas|动态链条总控 Canvas v1.0]]`：按重要性由中心向外排列动态链相关零件、依赖关系、完成度、归档与重开研究触发器。

新增统一原则：

```text
合法链 ≠ 优秀链

优秀动态边必须至少能说明：
mechanism + changed_variable Δ + residue + next_affordance
```

新增统一读法：

```text
横向：同阶段主 / 次 / 体量 / 生克补 / 依赖 / 主导权
纵向：trigger / Δ / 主次迁移 / residue / path / reversibility / next_affordance
```

---

## 当前有效案例
新增：`影片案例/楚门的世界_1998_帆船撞上摄影棚墙世界边界暴露与行动重组审计.md`

用途：记录楚门在自主离场方向已成立的前提下，因人工世界边界暴露而把行动从高风险直行重组为边界检查与出口寻找；不制造新的严格生克补边。

## 当前有效核心动态图
```text
阶段A：[主zx8｜次xz7｜none]
    ──船头撞墙、开放海域假定失效 / 从高风险直行改为边界检查与出口寻找──▶
阶段B：[主zx8｜次xn6｜none]
```

直接生克补边：`none`。

## 本轮失败边
```text
摄影棚墙 ──生──▶ zx
失败：楚门的离场方向在撞墙前已经成立。

xz ──生──▶ xn
失败：风险不自动生成检查与步骤组织。

zx ──生──▶ xn
未通过：同场连续不等于方向权直接生成流程组织。

xn ──补──▶ zx
失败：检查墙面与寻找出口没有补入新的方向目标。
```

## 后验累计
- 新增阶段对子迁移记录：`+1`。
- 新增`zx`方向持续并跨环境假定失效案例：`+1`。
- 新增`xn`现场行动重组案例：`+1`。
- 新增正式失败边：`+4`。
- 新增直接生克补关系：`0`。
- 理论关系计数：不变。
- `01-十元系统/`修改：`0`。

## 语义空间交接
```text
record_id：PTR-20260731-01-TRUMAN-WALL
validation_status：chain-proposed
阶段A：[主zx8｜次xz7｜none]
阶段B：[主zx8｜次xn6｜none]
transition_relation：none
七项最低：84
```

## 审计交接
本轮`audit_issue: none`。没有修改问题状态，也没有写理论卡。

## 旧影片审计下一验证
```text
楚门沿墙找到楼梯并登上，出口路径显现与离场步骤推进。
重点分账zx方向持续、xn现场步骤组织与现成设施条件；不得把楼梯或门写成十元端点。
```

## 新研发下一验证

按 `NEXT_动态链系列研究队列_20260917.yaml` 执行。当前研发优先级：

```text
R5 Evaluator跨题材盲测
→ ZX/NX 静态端点六图
→ 生克补首批8条T1
→ Dynamic Visual 10母型预检
→ 100样本盲测
→ 黎黎隆R6母链筛选
```


## 2026-09-20｜MUS 音乐动态链支线归档

- [[02_给我用的知识凝结库/01_动态链条/MUS-R6_生克补关系搭配消融_总结_20260920.md|MUS-R6 生克补关系搭配消融总结]]：固定黎黎隆角色与初始救援障碍，完成条件删除、顺序替换、关系方向、横向复合、作用范围五组**理论**对照；保留 changed_variable → residue → next_affordance 的状态继承规则。
- 状态：**理论方法已归档，实证与审美验证待做**。此处 MUS-R6 与本索引原有的 Evaluator R6 / 黎黎隆项目实战里程碑并非同一轮次，不改变原状态。


## 2026-09-21｜音乐验证延期 / Chat 优先

- [[02_给我用的知识凝结库/01_动态链条/MUS-R6_云端待办与Chat内闭环_20260921.md|MUS-R6 云端待办与 Chat 内闭环]]：外部音频、独立标注、受控听音、审美评测和分镜制作标为 deferred；聊天内先完成规则校验、关系组合与图结构逻辑消融。仅归档，不表示已启动云端计算或定时执行。

...
- `VERSION_动态链系列研发.yaml`：当前动态链系列所有子系统版本与下一里程碑。
...
- `[[02_给我用的知识凝结库/01_动态链条/00_动态链条总控_零件关系与完成度.canvas|动态链条总控 Canvas v1.0]]`：按重要性由中心向外排列动态链相关零件、依赖关系、完成度、归档与重开研究触发器。
...
用途：记录楚门在自主离场方向已成立的前提下，因人工世界边界暴露而把行动从高风险直行重组为边界检查与出口寻找；不制造新的严格生克补边。
...
zx ──生──▶ xn
未通过：同场连续不等于方向权直接生成流程组织。

xn ──补──▶ zx
失败：检查墙面与寻找出口没有补入新的方向目标。
```

## 后验累计
- 新增阶段对子迁移记录：`+1`。
- 新增`zx`方向持续并跨环境假定失效案例：`+1`。
- 新增`xn`现场行动重组案例：`+1`。
- 新增正式失败边：`+4`。
- 新增直接生克补关系：`0`。
- 理论关系计数：不变。
...
## 2026-09-20｜MUS 音乐动态链支线归档

- [[02_给我用的知识凝结库/01_动态链条/MUS-R6_生克补关系搭配消融_总结_20260920.md|MUS-R6 生克补关系搭配消融总结]]：固定黎黎隆角色与初始救援障碍，完成条件删除、顺序替换、关系方向、横向复合、作用范围五组**理论**对照；保留 changed_variable → residue → next_affordance 的状态继承规则。
- 状态：**理论方法已归档，实证与审美验证待做**。此处 MUS-R6 与本索引原有的 Evaluator R6 / 黎黎隆项目实战里程碑并非同一轮次，不改变原状态。

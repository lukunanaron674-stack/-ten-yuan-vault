...
把 GitHub 中的角色卡、场景文字索引和十元动态链，编译成云端 Work 可执行的 H3 视频任务；新图像文件只上传到云端，GitHub 保存供 Chat 选择的文字描述与云端路径/哈希。
...
- 新一批素材在 GitHub 只登记文字元数据：稳定 scene ID、场景描述、景别/构图/可见机制、适用方向、候选状态，以及云端路径和 SHA-256；不上传新的图片二进制到 GitHub。
- 图片原件上传到 RTX 4090D 的约定素材目录。Chat B 根据文字描述判断是否采用，并在任务中返回所选 scene ID；未被选择的素材不应自动进入生产任务。
- B 端生成任务必须保留所选记录中的 `scene_id`、`semantic_description`、`cloud_4090d_path`、`cloud_4090d_sha256`。C 端按 ID 定位云端图并先核验哈希，再绑定到任务。
- 路径缺失或哈希不符时标记为阻塞，不得用旧图替代。既有已上传图片作为历史资产保留，不因本规则追溯删除。

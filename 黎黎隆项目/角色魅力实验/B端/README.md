...
## v1.3 当前景别与画风规则（2026-09-24）

新轮次在 v1.2 单角色非MV、氛围音/情绪音、同prompt两抽、Work自行绑图验证的基础上，必须读取 `rules/video_tenyuan_prompt_schema_v1.3.md`。V35 1194字符文本仅作文字风格参考，已有workflow存在沿用澈旧词/旧图的审计疑点，不能把V35成片当作这份新词的已验证成效。STYLE区分身份画风锁（二维手绘清晰线面、轻薄晕染等以原图为准）与镜头表现变量；Picture 2只锁同一场景身份、空间和色卡逻辑，不锁原图景别/机位/构图。允许按动态链另写景别、机位、前中后景、构图留白、天气雾与光照变化；角色Picture 1始终保留身份，只有一个出镜人物，不得复制分身。不能凭文字幻想参考图里未核实的物件和空间，必要时标记needs_environment_inference。最终H3正文加 `CAMERA AND COMPOSITION` 并明确每个镜头从起始动作到结束的连续关系。

...
B端角色素材只有 `LLL-MAIN-001`（黎黎隆本人）和唯一角色图 `assets/character_refs/LLL_H3_character_card_001.jpg`。所有历史非黎黎隆任务仅作归档，不应由云端Work继续消费；已经排队的非黎黎隆任务应由云端侧标为取消或忽略，不得作为新任务依据。场景保持独立的13张正式场景图池；R3预跑图已从B端角色图目录清理，不得以旧路径重新引用。

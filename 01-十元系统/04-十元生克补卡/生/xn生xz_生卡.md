---
type: ten-yuan-single-relation-card
relation_kind: 生
relation: xn 生 xz
source: xn
target: xz
relation_status: locked
status: canonical-current-card
version: v0.1
card_schema_version: v1.1
memory_record_version: v1.0
last_memory_run_id: null
last_memory_commit: null
updated: 2026-07-28
relation_table_version: v2.22
primary_hourly_write_target: true
source_information_card: 01-十元系统/【xn信息量卡v2】.md
target_information_card: 01-十元系统/【xz信息量卡v2】.md
source_accuracy_card: 01-十元系统/03-十元准度卡/xn_准度卡_v0.1.md
target_accuracy_card: 01-十元系统/03-十元准度卡/xz_准度卡_v0.2.md
canonical_specialty: 01-十元系统/关系专项/xn生xz_结构把偏离转成临界风险正式机制_20260719.md
---

# xn 生 xz｜单条生卡

## 当前锁定机制

`xn`规定正常时间窗、路线与身份接口；偏差被识别、记录并持续处理，安全路径、退路和可用时间逐步减少，`xz`的危险可到达性与临界风险上升。

## 被改变变量

偏差识别率、追踪概率、可用路线、时间窗、退路宽度与稳定停留可能性。

## 红线

普通守规则、紧张气氛、倒计时文字、一次处罚或危险尚不可到达，均不足。

## 固定测试

- 最近邻：`zx`主动追击、自然灾害、普通程序与视觉紧迫。
- 拿掉：拿掉结构识别和持续处理后，若危险仍以同样路径逼近，则不支持本关系。
- 反向：关闭记录、追踪和接口处理后，安全路线或时间窗应恢复。
- 第三因素：外部敌人、自然危险、资源耗尽、主体自曝与偶然事故。

## 七项置信度与案例

新增片段、失败边、七项评分与边界全部原位写本卡；专项正本保持正式机制权威。

## 记忆记录

每次有效运行记录本卡理论版本、记忆记录版本、run_id与commit SHA。